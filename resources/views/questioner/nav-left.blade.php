  <div class="inside-sidemenu">
    <div class="box">
      <!-- <div class="box-header with-border">
        <h4>
          <i class="fa fa-gear fa-fw"></i>&nbsp;Setting
          <span class="pull-right">
            <i class="fa fa-sort-amount-asc"></i>
          </span>
        </h4>
      </div> -->
      <div class="box-body">
        <ul class="list-group inside-submenu no-margin" data-widget="tree" style="list-style: none;">
          <li class="">
            <a href="{{route('quisioner.list')}}">
              <h4><i class="fa fa-list fa-fw"></i>&nbsp;<span>List Quisioner</span></h4>
            </a>
          </li>
          <li class="">
            <!-- <a id="mn_create_new_questioner" href="javascript:void(0)"> -->
            <a href="{{route('quisioner.create-new')}}">
              <h4><i class="fa fa-clone"></i>&nbsp;<span>New Quisioner</span></h4>
            </a>
          </li>
          <li class="">
            <a href="#">
              <h4><i class="fa fa-calculator"></i>&nbsp;<span>Preview & Score</span></h4>
            </a>
          </li>

          <!-- <li class="">
            <a href="#">
              <h4><i class="fa fa-bell"></i>&nbsp;&nbsp;<span>Notifications</span></h4>
            </a>
          </li>
          <li class="">
            <a href="{{route('setting.users')}}">
              <h4><i class="fa fa-users"></i>&nbsp;&nbsp;<span>Users</span></h4>
            </a>
          </li>
          <li class="">
            <a href="{{route('setting.blackwhitelist')}}">
              <h4><i class="fa fa-ban"></i>&nbsp;<span>Blacklist and Whitelist</span></h4>
            </a>
          </li> -->
          
          <!-- <li class="footer">
            <div class="footer-tools">
              <a href="#">
                <i class="fa fa-plus-circle fa-fw"></i>
              </a>
              <a href="#">
                <i class="fa fa-folder fa-fw"></i>
              </a>
              <a href="#">
                <i class="fa fa-sticky-note-o fa-fw"></i>
              </a>
              <a href="#">
                <i class="fa fa-upload fa-fw"></i>
              </a>
            </div>
          </li> -->

        </ul>
      </div>
    </div>
  </div>
  